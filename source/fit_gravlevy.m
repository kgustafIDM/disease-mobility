function [modelg, modelg23, modelg2, modelpow, aX3cdflessfit, aX3cdfmorefit, likliplcdfless, likliplcdfmore, cpow] = fit_gravlevy(rho,tau1,tau2,alevy,EVDpop,EVDroaddist,nonlinfits)
...
if nonlinfits
    ...
    betagrav = [1 1 1 -2];
    modelg = fitnlm(logfitX(:,minbinEVD:end)',logcounts(minbinEVD:end),modelfun_gravity,betagrav,'Options',opts);
    modelg.Coefficients;
    
    modelfun_gravity23 = @(b,x)b(1) + x(:,1).*tau1 + x(:,2).*b(2) + x(:,3).*b(3);
    betagrav = [1 1 -2];
    modelg23 = fitnlm(logfitX(:,minbinEVD:end)',logcounts(minbinEVD:end),modelfun_gravity23,betagrav,'Options',opts);
    modelg23.Coefficients;
    ...
    modelg = 0; modelg23 = 0; modelg2 = 0; modelpow = 0;
end
